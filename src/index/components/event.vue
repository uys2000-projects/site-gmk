<template>
  {{ events.length != 0 ? conterEvent : "" }}
  <a v-if="events.length != 0"></a>
  <q-carousel
    v-model="slide"
  ...
  >
    ...
    >
      <div class="q-col-gutter-md row items-start">
        ...
        >
          <q-img
            @click="openPage(events[i - 1 + n * 2].header)"
            :src="events[i - 1 + n * 2].headerImage"
            no-native-menu
            :style="`width:100%; height:${wH * 0.73}px; object-fit: cover;`"
          >
            <div class="absolute-bottom text-subtitle1 text-center">
              {{ events[i - 1 + n * 2].header }}
            </div>
          </q-img>
        </div>
      </div>
...
export default {
  ...
  methods: {
    conetrFunc: function () {
      this.conter = this.events.length;
    },
    openPage: function (item) {
      this.alert = true;
      this.header = item;
      getDocument("event", item).then((res) => {
    ...
    conter(val) {
      ...
      for (var itemNumber = 0; itemNumber < val; itemNumber++) {
        if (itemNumber % 2 == 0) {
          ...
          this.itemCounts[this.slideCount - 1] = 2;
        }
        itemNumber;
      }
    },
